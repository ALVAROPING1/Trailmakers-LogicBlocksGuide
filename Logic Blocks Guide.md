...
- Keybinds: see \nameref{keybinds}
- Toggle: see \nameref{toggle}
- Timers: see \nameref{timers} <!-- NOTE: currently does nothing with use steps, and without it stored value changes regardless of them even though output value doesn't. Without use steps, if the pause is 0 but duration isn't, the duration timer is also ignored. I assume this is a bug -->
...
Number displays show and output the sum of their inputs with an optional rounding, similar to how OR logic gates work. If the result of the sum is outside of the $[-1000, 1000)$ range, it's replaced with a "N/A" value.
